...
new MutationObserver(()=>{const tag=[...document.querySelectorAll('#healthContent+.tag,.toolbar .tag')].find(x=>/^Build\s/i.test(x.textContent||''));if(tag&&tag.textContent!=='Build 2587')tag.textContent='Build 2587'}).observe(document.body,{childList:true,subtree:true});
})();
