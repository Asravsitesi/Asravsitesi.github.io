(()=>{
  ...
  style.textContent=`
    ...
    #forgotPanel{margin-top:14px;padding:14px;border:1px solid #dfe5eb;border-radius:12px;background:#f8fafc}#forgotPanel .field{margin-bottom:10px}#forgotPanel .forgot-actions{display:flex;gap:8px}#forgotPanel .forgot-actions button{flex:1}#forgotPassword{width:100%;margin-top:10px;color:#245b70}#recoveryForm .field{margin-bottom:14px}
    @media(max-width:760px){html body #app #content table#rows{min-width:1180px!important}}
  `;
  document.head.appendChild(style);

  const flow=authSearch.get('flow')||authHash.get('flow')||authFlow;
  const hasCallback=authSearch.has('code')||authHash.has('access_token')||authHash.has('token_hash');
  const pendingAt=Number(localStorage.getItem('asravRecoveryPendingAt')||0);
  const recoveryFlow=flow==='recovery'||authType==='recovery'||(hasCallback&&pendingAt>0&&Date.now()-pendingAt<60*60*1000);
  const signupFlow=flow==='signup'||authType==='signup';
  let recoveryRendered=false,signupHandled=false;
  if(recoveryFlow||signupFlow)authNoticeShown=true;

  function showRecoveryWaiting(message='Şifre sıfırlama bağlantısı doğrulanıyor…'){
    document.querySelector('#app').classList.add('hidden');document.querySelector('#login').classList.remove('hidden');
    const card=document.querySelector('#loginForm');
    if(!recoveryRendered)card.innerHTML=`<h2>Bağlantı doğrulanıyor</h2><p class="sub" id="recoveryWaiting">${message}</p>`;
  }
  function showRecovery(){
    if(recoveryRendered)return;recoveryRendered=true;authNoticeShown=true;
    document.querySelector('#app').classList.add('hidden');document.querySelector('#login').classList.remove('hidden');
    const card=document.querySelector('#loginForm');card.innerHTML=`<h2>Yeni şifrenizi belirleyin</h2><p class="sub">Bu bağlantı yalnızca yeni şifre belirlemek için kullanılabilir. Uygulamaya erişim verilmez.</p><div id="recoveryForm"><div class="field"><label>Yeni şifre</label><input id="newPassword" type="password" minlength="8" maxlength="128" autocomplete="new-password"></div><div class="field"><label>Yeni şifre tekrar</label><input id="newPasswordAgain" type="password" minlength="8" maxlength="128" autocomplete="new-password"></div><button class="primary" type="button" id="saveNewPassword" style="width:100%">Yeni şifreyi kaydet</button></div>`;
    history.replaceState({},'',location.pathname+'?flow=recovery');
    document.querySelector('#saveNewPassword').onclick=async()=>{const a=document.querySelector('#newPassword').value,b=document.querySelector('#newPasswordAgain').value,btn=document.querySelector('#saveNewPassword');if(a.length<8)return toast('Yeni şifre en az 8 karakter olmalıdır.','error');if(a!==b)return toast('Şifreler birbiriyle eşleşmiyor.','error');btn.disabled=true;const {error}=await db.auth.updateUser({password:a});if(error){btn.disabled=false;return toast('Şifre güncellenemedi. Bağlantının süresi dolmuş olabilir.','error')}localStorage.removeItem('asravRecoveryPendingAt');toast('Şifreniz değiştirildi. Yeni şifrenizle giriş yapabilirsiniz.','success',7000);await db.auth.signOut();history.replaceState({},'',location.pathname);setTimeout(()=>location.reload(),1200)};
  }

  const originalBoot=boot;
  boot=async function(s){
    if(recoveryFlow){session=s;showRecoveryWaiting();if(s)showRecovery();return}
    if(signupFlow&&hasCallback&&!signupHandled){signupHandled=true;authNoticeShown=true;if(s)await db.auth.signOut();document.querySelector('#app').classList.add('hidden');document.querySelector('#login').classList.remove('hidden');history.replaceState({},'',location.pathname);toast('E-posta adresiniz doğrulandı. Güvenliğiniz için şifrenizle giriş yapın.','success',8000);return}
    return originalBoot(s);
  };
  if(recoveryFlow)showRecoveryWaiting();
  db.auth.onAuthStateChange((event,s)=>{if(recoveryFlow&&(event==='PASSWORD_RECOVERY'||event==='SIGNED_IN')&&s)showRecovery()});
  if(recoveryFlow)setTimeout(()=>{if(!recoveryRendered&&!session){const el=document.querySelector('#recoveryWaiting');if(el)el.textContent='Bağlantı geçersiz veya süresi dolmuş. Giriş sayfasından yeni bir bağlantı isteyin.'}},8000);

  const toggle=document.querySelector('#authToggle'),submit=document.querySelector('#authSubmit');
  if(!toggle||!submit||recoveryFlow)return;
  ...
  sendBtn.onclick=async()=>{const email=emailInput.value.trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return toast('Geçerli bir e-posta adresi girin.','error');sendBtn.disabled=true;localStorage.setItem('asravRecoveryPendingAt',String(Date.now()));const {error}=await db.auth.resetPasswordForEmail(email,{redirectTo:AUTH_RECOVERY_REDIRECT});sendBtn.disabled=false;if(error){localStorage.removeItem('asravRecoveryPendingAt');const m=String(error.message||'');if(/rate|security purposes/i.test(m))return toast('Kısa süre önce bağlantı gönderildi. Lütfen en az 60 saniye bekleyin.','error',7000);return toast('İstek tamamlanamadı. Lütfen daha sonra tekrar deneyin.','error')}toast('Bu adres sistemde kayıtlıysa şifre sıfırlama bağlantısı gönderildi. Gelen kutusu ve spam klasörünü kontrol edin.','success',9000)};
  const originalSetAuthMode=setAuthMode;setAuthMode=function(v){originalSetAuthMode(v);forgotBtn.classList.toggle('hidden',v);if(v)panel.classList.add('hidden')};
})();
