...
async function loadAll(){try{const uid=session.user.id;let {data:p,error}=await db.from('profiles').select('*').eq('id',uid).single();if(error)throw error;profile=p;const queries=[db.from('units').select('*').order('id'),db.from('profiles').select('*').order('full_name'),db.from('dues').select('*').order('period',{ascending:false}),db.from('budgets').select('*').order('year',{ascending:false}),db.from('announcements').select('*').order('created_at',{ascending:false}),db.from('documents').select('*').order('created_at',{ascending:false})];const r=await Promise.all(queries);['units','profiles','dues','budgets','announcements','documents'].forEach((k,i)=>{if(r[i].error)throw r[i].error;data[k]=r[i].data||[]});$('#userName').textContent=profile.full_name||session.user.email;$('#userRole').textContent=roles[profile.role]+' · '+unit(profile.unit_id);$('#avatar').textContent=(profile.full_name||'AS').split(' ').map(x=>x[0]).slice(0,2).join('').toUpperCase();$$('.manager-only').forEach(x=>x.classList.toggle('hidden',!isManager()));$('#syncStatus').textContent='● Güncel';$('#syncStatus').className='tag green';render()}catch(e){console.error(e);toast('Veriler yüklenemedi: '+e.message)}}
...
function render(){const titles={dashboard:'Genel Bakış',dues:'Aidatlar',budgets:'Bütçe',docs:'Belgeler',notices:'Duyurular',residents:'Sakinler',settings:'Hesabım'};$('#pageTitle').textContent=titles[page];$('#content').innerHTML=({dashboard,dues,budgets,docs,notices,residents,settings}[page]||dashboard)();bind()}
